...
from nuscenes.utils.geometry_utils import BoxVisibility, box_in_image, transform_matrix, view_points
...
class NuScenes:
    ...
    def __init__(
        ...
        for i, lidar_task in enumerate(lidar_tasks):
            ...
            self.colormap = dict(
                {c["name"]: self.colormap[c["name"]] for c in sorted(self.category, key=lambda k: k["index"])}
            )
    ...
    def get_sample_data(
    ...
    ) -> Tuple[str, List[Box], Optional[np.ndarray]]:
        """
        ...
        for box in boxes:
            ...
            if cam_intrinsic and imsize and not box_in_image(box, cam_intrinsic, imsize, vis_level=box_vis_level):
                continue
    ...
    def get_sample_lidarseg_stats(
        self, sample_token: str, sort_by: str = "count", lidarseg_preds_bin_path: str = None, gt_from: str = "lidarseg"
    ) -> None:
        """
        ...
        assert hasattr(self, gt_from), (
            f"Error: You have no {gt_from} data; unable to get statistics for segmentation of the point cloud."
        )
        ...
        if lidarseg_preds_bin_path:
            ...
            lidarseg_labels_filename = os.path.join(self.dataroot, self.get(gt_from, lidar_sd_token)["filename"])
    ...
    def render_scene(
        self, scene_token: str, freq: float = 10, imsize: Tuple[float, float] = (640, 360), out_path: str = None
    ) -> None:
...
class NuScenesExplorer:
    ...
    def list_scenes(self) -> None:
        ...
        recs = [
            (self.nusc.get("sample", record["first_sample_token"])["timestamp"], record) for record in self.nusc.scene
        ]
    ...
    def map_pointcloud_to_image(
        ...
        if render_intensity:
            assert pointsensor["sensor_modality"] == "lidar", (
                "Error: Can only render intensity for lidar, not %s!" % pointsensor["sensor_modality"]
            )
            ...
            assert pointsensor["sensor_modality"] == "lidar", (
                "Error: Can only render lidarseg labels for lidar, not %s!" % pointsensor["sensor_modality"]
            )
            ...
            if lidarseg_preds_bin_path:
                ...
                if len(semantic_table) > 0:  # Ensure {lidarseg/panoptic}.json is not empty (e.g. in case of v1.0-test).
                    lidarseg_labels_filename = osp.join(
                        self.nusc.dataroot, self.nusc.get(gt_from, pointsensor_token)["filename"]
                    )
                else:
            ...
            if sensor_modality == "lidar" and (show_lidarseg or show_panoptic):
                ...
                if lidarseg_preds_bin_path:
                    ...
                    if len(semantic_table) > 0:
                        ...
                        lidarseg_labels_filename = osp.join(
                            self.nusc.dataroot, self.nusc.get(gt_from, sample_data_token)["filename"]
                        )
                    else:
                        lidarseg_labels_filename = None
    ...
    def render_annotation(
        ...
        assert len(boxes) > 0, (
            "Error: Could not find image where annotation is visible. Try using e.g. BoxVisibility.ANY."
        )
        ...
        if with_anns:
            ...
            for box in boxes:
                ...
                if (
                    ...
                    and self.nusc.lidarseg_name2idx_mapping[box.name] not in filter_lidarseg_labels
                ):
                    continue
                c = self.get_color(box.name)
    ...
    def render_scene_channel_lidarseg(
        ...
        if save_as_vid:
            assert total_num_samples == i, (
                "Error: There were supposed to be {} keyframes, but only {} keyframes were processed".format(
                    total_num_samples, i
                )
            )
            out.release()
